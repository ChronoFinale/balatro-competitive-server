...
public final class Val {
    ...
    public static Value runVar(Value.Var v, double base, double scale) { return new Value.RunVar(v, base, scale); }

    /** {@code scale} per unit of a run variable (Banner: +30 Chips per remaining discard). */
    public static Value perRunVar(Value.Var v, double scale) { return new Value.RunVar(v, 0, scale); }

    /** {@code 1 + scale} per unit — the x-Mult convention over a run variable (Obelisk, Throwback). */
    public static Value xPerRunVar(Value.Var v, double scale) { return new Value.RunVar(v, 1, scale); }

    /** {@code base + scale * floor(runVar / per)} — stepwise run-var scaling (Bootstraps: +2 per $5). */
    public static Value runVarStep(Value.Var v, double base, double scale, double per) {
