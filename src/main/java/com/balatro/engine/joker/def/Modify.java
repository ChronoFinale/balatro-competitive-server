...
/**
 * A modifier on a game {@link Subject} — the <i>write</i> half of the same vocabulary conditions <i>read</i>.
 * A condition reads {@code MONEY} ({@code money >= 5}); a modifier writes it ({@code add(MONEY, 4)}).
 * Same nouns, opposite verbs. Targets a {@link Subject} (a {@link Hand} noun like {@code Hand.SIZE}, or a
 ...
 */
public record Modify(Subject variable, Op op, double value) {

    public enum Op { ADD, SET, MULTIPLY, MAX, MIN }

    public static Modify add(Subject variable, double value) { return new Modify(variable, Op.ADD, value); }

    public static Modify set(Subject variable, double value) { return new Modify(variable, Op.SET, value); }

    public static Modify multiply(Subject variable, double value) { return new Modify(variable, Op.MULTIPLY, value); }

    /** Raise the value to at least {@code value} — the highest tier wins (Seed Money/Money Tree cap, Overstock slots). */
    public static Modify max(Subject variable, double value) { return new Modify(variable, Op.MAX, value); }

    /** Lower the value to at most {@code value} — the deepest wins (Clearance/Liquidation price; Green's no-interest cap 0). */
    public static Modify min(Subject variable, double value) { return new Modify(variable, Op.MIN, value); }
     ...
     */
    public static double fold(double base, Subject variable, List<Modify> mods) {
        double v = base;
