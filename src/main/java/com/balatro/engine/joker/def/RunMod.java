...
public record RunMod(boolean disablesBoss, double survivesLostBlindFraction,
    ...
    public static final RunMod NONE = new RunMod(false, 0, false, 0, false);

    private static RunMod capability(boolean disablesBoss, double survivesFraction,
            boolean doublesProbability, int handSizeDecayStart, boolean pvpSkipBonus) {
        return new RunMod(disablesBoss, survivesFraction, doublesProbability, handSizeDecayStart, pvpSkipBonus);
    }

    /** Chicot: while owned, every Boss Blind's ability is disabled. */
    public static RunMod bossDisabler() {
        return capability(true, 0, false, 0, false);
    }
