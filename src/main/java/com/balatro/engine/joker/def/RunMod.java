 ...
 * {@code disablesBoss} (Chicot), {@code survivesLostBlindFraction} (Mr Bones), {@code doublesProbability}
 ...
 * {@code pvpSkipBonus} (Skip Off), and the {@link OnSell} group (Invisible). None affect the per-hand score.
 *
 * <p>(Perkeo/Luchador/Diet Cola/Madness/Ceremonial are data RULES now — SHOP_EXIT / SELL_SELF /
 * BLIND_SELECTED — not capabilities.)
 */
public record RunMod(boolean disablesBoss, double survivesLostBlindFraction, OnSell onSell,
    ...
    public static final RunMod NONE = new RunMod(false, 0, OnSell.NONE, false, 0, 0, false);
    ...
    private static RunMod capability(boolean disablesBoss, double survivesFraction, OnSell sell,
            boolean doublesProbability, int handSizeDecayStart, int pvpShopSpendDenominator, boolean pvpSkipBonus) {
        return new RunMod(disablesBoss, survivesFraction, sell, doublesProbability,
    ...
    public static RunMod bossDisabler() {
        return capability(true, 0, OnSell.NONE, false, 0, 0, false);
    }
    ...
    public static RunMod survivesLostBlind(double fraction) {
        return capability(false, fraction, OnSell.NONE, false, 0, 0, false);
    }

    /** Oops! All 6s: doubles every listed probability while owned (stacks per copy). */
    public static RunMod probabilityDoubler() {
        return capability(false, 0, OnSell.NONE, true, 0, 0, false);
    }
    ...
    public static RunMod decayingHandSize(int start) {
        return capability(false, 0, OnSell.NONE, false, start, 0, false);
    }
