...
import static com.balatro.engine.TestSupport.poly;
...
class ScoringEngineTest {
    ...
    void jokerEditions() { // Foil +50 chips, Holo +10 mult (both before the joker scores), Poly x1.5 after
        ...
        assertThat(editionScore(com.balatro.engine.card.Edition.FOIL, played).score()).isEqualTo(80.0 * 6); // (30+50) x 6
        assertThat(editionScore(com.balatro.engine.card.Edition.HOLOGRAPHIC, played).score()).isEqualTo(30.0 * 16); // 30 x (6+10)
        assertThat(editionScore(com.balatro.engine.card.Edition.POLYCHROME, played).score()).isEqualTo(180.0 * 1.5); // 30 x 6 x1.5
    }

    private static ScoreResult editionScore(com.balatro.engine.card.Edition ed, List<Card> played) {
        RunState run = new RunState();
        var j = com.balatro.engine.joker.JokerLibrary.create("j_joker");
        run.addJoker(j);
    ...
    @Test
    void blueprintChainsThroughAnotherBlueprint() {
        // Blueprint -> Blueprint -> Joker: the copiers are themselves copyable (real Balatro), so the chain
        // resolves recursively and +4 Mult applies three times -> 30 x (2 base + 4+4+4) = 30 x 14 = 420.
        assertThat(score(jokers("j_blueprint", "j_blueprint", "j_joker"),
                List.of(c(KING, HEARTS), c(KING, SPADES))).score())
                .isEqualTo(420.0);
    }
    ...
    void roughGemCreditsMoneyOnScoreButNotOnPreview() {
        RunState run = new RunState();
        run.money = 10;
        run.addJoker(com.balatro.engine.joker.JokerLibrary.create("j_rough_gem"));
        RandomStreams rng = new RandomStreams("GEM");
        // two played Diamonds -> +$2; the other three cards earn nothing
        List<Card> hand = List.of(c(NINE, com.balatro.engine.card.Suit.DIAMONDS),
                c(NINE, com.balatro.engine.card.Suit.DIAMONDS), c(THREE, SPADES),
    ...
    void goldenTicketPaysForScoredGoldCards() {
        RunState run = new RunState();
        run.money = 0;
        run.addJoker(com.balatro.engine.joker.JokerLibrary.create("j_golden_ticket"));
        RandomStreams rng = new RandomStreams("GT");
        com.balatro.engine.card.Card gold = new com.balatro.engine.card.Card(
                KING, HEARTS, com.balatro.engine.card.Enhancement.GOLD,
                com.balatro.engine.card.Edition.NONE, com.balatro.engine.card.Seal.NONE);
    ...
    void greenJokerGrowsPerHandAndShrinksPerDiscard() {
        RunState run = new RunState();
        var gj = com.balatro.engine.joker.JokerLibrary.create("j_green_joker");
        run.addJoker(gj);
        ...
        com.balatro.engine.game.GameEvents.preDiscard(run, rng, List.of(c(THREE, SPADES)));
    ...
    void hangingChadHasASinglePlayerAndMultiplayerVariant() {
        ...
        sp.addJoker(com.balatro.engine.joker.JokerLibrary.create("j_hanging_chad", "default"));
        RunState mp = new RunState();
        mp.addJoker(com.balatro.engine.joker.JokerLibrary.create("j_hanging_chad", "multiplayer"));
    ...
    void rideTheBusScalesAcrossConsecutiveHands() {
        RunState run = new RunState();
        run.addJoker(com.balatro.engine.joker.JokerLibrary.create("j_ride_the_bus"));
