...
class StickerTest {
    ...
    void rentalJokerChargesThreeDollarsPerRound() {
        // Small blind won in 1 hand: +$3 reward + $3 ($1 × 3 remaining hands), $0 interest at $4 = +$6.
        ...
        assertThat(rental.state.money).isEqualTo(before + 3); // +$6 economy − $3 rent = +$3

        Run normal = new Run(STD, "R", stoneDeck(300), jokers("j_joker", "j_joker", "j_joker"));
        int before2 = normal.state.money;
        normal.play(FIVE);
        assertThat(normal.state.money).isEqualTo(before2 + 6); // $3 reward + $3 remaining-hand money, no rent
    }
}
